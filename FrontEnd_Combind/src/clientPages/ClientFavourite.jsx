...
const ClientFavourite = () => {
	...
	useEffect(() => {
        const favouriteAuth = async () => {
            try {
                const response = await axios.get(`${BACK_END_BASE_URL}/dashboard/customer/${authData.clientData.clientID}/favorite`, { 
                ...
                if (response.status === 200 && response.data.favorites) {
                    ...
                    console.log(typeof(response.data.favorites));

                    setFavouriteData(
                        response.data.favorites.map(item => ({
                            id: item.id || "",
                            name: item.name || "",
                            image: item.image || null,
                            rating: item.rating ? {
                                number_of_reviews: item.rating.number_of_reviews || 0,
                                average: item.rating.average || 0.0
                            } : {
                                number_of_reviews: 0,
                                average: 0.0
                            },
            ...
            <div className="container">
                {favouriteData.map((stall, index) => (
                    <div className="item-container" key={index}>
                        ...
                        <div className="name-container">{stall.name && <p>{stall.name}</p>}</div>
                        <div className="rating-container">
                            ...
                            <p key={index}>{stall.rating ? `${stall.rating.average.toFixed(1)} (${stall.rating.number_of_reviews} reviews)`: "--"}</p>
                        </div>
                    </div>
                ))}
            </div>
