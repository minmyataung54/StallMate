...
const OrderDetail = ({ detail, setDetail, order, profile }) => {
  ...
  const handleCompletedBtn = async () => {
    try {
      const response = await axios.put(
        `${BACK_END_BASE_URL}/dashboard/stallowner/${authData?.ownerData.ownerID}/orders/${order.orderId}/completed`,
        {},
        {
  ...
  return (
    <div className="container-fluid d-flex flex-column pd-0 text-white">
      ...
      {/* Restaurant Details */}
      <div className="row d-flex">
        <img className="col-4" src={profile.restaurant.photo} alt="Restaurant" />
        <h5 className="col-8 text-white align-self-center">{profile.restaurant.name}</h5>
      </div>
      {/* Menu Items */}
